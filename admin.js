...
import { collection, doc, addDoc, setDoc, updateDoc, onSnapshot, query, where, serverTimestamp, runTransaction } from 'https://www.gstatic.com/firebasejs/12.3.0/firebase-firestore.js';
...
let users=[],levels=[],deposits=[],withdrawals=[],investments=[],exchanges=[],chatUnsub=null;
...
function renderAll(){renderMetrics();renderUsers();renderLevels();renderRequests();renderSelectors();renderFlows();renderRecent()}
...
function renderUsers(){
  const list=users.filter(u=>u.uid!==ADMIN_UID);
...
function renderSelectors(){
  const opts=users.filter(u=>u.uid!==ADMIN_UID).map(u=>`<option value="${u.uid}">${esc(u.name||u.email)} — ${esc(u.email||'')}</option>`).join('');
...
function pendingRequests(){
  return [
    ...deposits.filter(x=>x.status==='pending').map(x=>({...x,_kind:'deposit',_label:'Dépôt'})),
    ...withdrawals.filter(x=>x.status==='pending').map(x=>({...x,_kind:'withdrawal',_label:'Retrait'})),
    ...investments.filter(x=>x.status==='pending').map(x=>({...x,_kind:'investment',_label:'Investissement'})),
    ...exchanges.filter(x=>x.status==='pending').map(x=>({...x,_kind:'exchange',_label:'Échange'}))
...
async function rejectRequest(kind,id){if(!confirm('Rejeter cette demande ?'))return;try{await updateDoc(doc(db,getCollection(kind),id),{status:'rejected',processedAt:serverTimestamp(),processedBy:ADMIN_UID});toast('Demande rejetée.')}catch(e){console.error(e);toast('Erreur.')}}

async function approveRequest(kind,id){
  const req=getArr(kind).find(x=>x.id===id);if(!req)return;
  let reference='';let rate=null;
  ...
  try{
    await runTransaction(db,async tx=>{
      const reqRef=doc(db,getCollection(kind),id),userRef=doc(db,'users',req.userId);const [reqSnap,userSnap]=await Promise.all([tx.get(reqRef),tx.get(userRef)]);if(!reqSnap.exists()||!userSnap.exists())throw new Error('Document introuvable');if(reqSnap.data().status!=='pending')throw new Error('Déjà traité');
      const u=userSnap.data(),balances={...(u.balances||{})};
      if(kind==='deposit'){const c=req.currency;balances[c]=Number(balances[c]||0)+Number(req.amount);tx.update(reqRef,{status:'approved',adminReference:reference,processedAt:serverTimestamp(),processedBy:ADMIN_UID});}
      if(kind==='withdrawal'){const c=req.currency;if(Number(balances[c]||0)<Number(req.amount))throw new Error('Solde insuffisant');balances[c]=Number(balances[c]||0)-Number(req.amount);tx.update(reqRef,{status:'paid',adminReference:reference,processedAt:serverTimestamp(),processedBy:ADMIN_UID});}
      if(kind==='investment'){const c=req.currency;if(Number(balances[c]||0)<Number(req.amount))throw new Error('Solde insuffisant');balances[c]=Number(balances[c]||0)-Number(req.amount);tx.update(reqRef,{status:'approved',approvedAt:serverTimestamp(),processedBy:ADMIN_UID});}
      if(kind==='exchange'){const from=req.fromCurrency,to=req.toCurrency;if(Number(balances[from]||0)<Number(req.amount))throw new Error('Solde insuffisant');const converted=Number(req.amount)*rate;balances[from]=Number(balances[from]||0)-Number(req.amount);balances[to]=Number(balances[to]||0)+converted;tx.update(reqRef,{status:'approved',rate,convertedAmount:converted,adminReference:reference,processedAt:serverTimestamp(),processedBy:ADMIN_UID});}
      tx.update(userRef,{balances,updatedAt:serverTimestamp()});
    });
    toast('Opération validée.');
  }catch(e){console.error(e);toast(e.message||'Validation impossible.');}
}

$('#manualOperationForm').onsubmit=async e=>{e.preventDefault();const uid=$('#manualUser').value,type=$('#manualType').value,amount=Number($('#manualAmount').value),currency=$('#manualCurrency').value,method=$('#manualMethod').value,reference=$('#manualReference').value.trim();if(!uid||!(amount>0)||!reference)return;try{
  await runTransaction(db,async tx=>{const userRef=doc(db,'users',uid),snap=await tx.get(userRef);if(!snap.exists())throw new Error('Investisseur introuvable');const balances={...(snap.data().balances||{})};if(type==='withdrawal'&&Number(balances[currency]||0)<amount)throw new Error('Solde insuffisant');balances[currency]=Number(balances[currency]||0)+(type==='deposit'?amount:-amount);tx.update(userRef,{balances,updatedAt:serverTimestamp()});const col=type==='deposit'?'deposits':'withdrawals';const opRef=doc(collection(db,col));tx.set(opRef,{userId:uid,userEmail:snap.data().email,userName:snap.data().name,amount,currency,method,reference,adminReference:reference,status:type==='deposit'?'approved':'paid',source:'admin-manual',createdAt:serverTimestamp(),processedAt:serverTimestamp(),processedBy:ADMIN_UID});});e.target.reset();toast('Opération manuelle enregistrée.');
}catch(err){console.error(err);toast(err.message||'Opération impossible.')}};

function watchChat(uid){if(chatUnsub){chatUnsub();chatUnsub=null}if(!uid){$('#adminMessageList').innerHTML='<div class="empty">Sélectionnez un investisseur.</div>';return}chatUnsub=onSnapshot(query(collection(db,'messages'),where('userId','==',uid)),snap=>{const arr=snap.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>stamp(a.createdAt)-stamp(b.createdAt));$('#adminMessageList').innerHTML=arr.length?arr.map(m=>`<div class="chat-bubble ${m.senderRole==='admin'?'admin':'investor'}"><strong>${m.senderRole==='admin'?'Admin':esc(users.find(u=>u.uid===uid)?.name||'Investisseur')}</strong><div>${esc(m.text)}</div><small>${stamp(m.createdAt).getTime()?stamp(m.createdAt).toLocaleString('fr-FR'):''}</small></div>`).join(''):'<div class="empty">Aucun message.</div>';$('#adminMessageList').scrollTop=$('#adminMessageList').scrollHeight;});}
$('#chatUserSelect').onchange=e=>watchChat(e.target.value);
$('#adminMessageForm').onsubmit=async e=>{e.preventDefault();const uid=$('#chatUserSelect').value,text=$('#adminMessageText').value.trim();if(!uid||!text){toast('Choisissez un investisseur.');return}await addDoc(collection(db,'messages'),{userId:uid,senderId:ADMIN_UID,senderRole:'admin',text,createdAt:serverTimestamp()});e.target.reset()};
...
function watch(name,setter){onSnapshot(collection(db,name),snap=>{setter(snap.docs.map(d=>({id:d.id,...d.data()})));renderAll()},e=>console.error(name,e))}

onAuthStateChanged(auth,u=>{
  if(!u){location.href='index.html';return}if(u.uid!==ADMIN_UID){location.href='investisseur.html';return}
  watch('users',x=>users=x);watch('levels',x=>levels=x);watch('deposits',x=>deposits=x);watch('withdrawals',x=>withdrawals=x);watch('investments',x=>investments=x);watch('exchangeRequests',x=>exchanges=x);
  setTimeout(()=>watchChat($('#chatUserSelect').value),1200);
});
